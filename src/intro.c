...
static void Task_Scene1_Load(u8 taskId)
{
    ...
    SetGpuBackgroundY(2, 80);
    SetGpuBackgroundY(1, 24);
    SetGpuBackgroundY(0, 40);
    LZ77UnCompVram(sIntro1Bg_Gfx, gpu.gfxData);
    ...
    CreateGameFreakLogoSprites(DisplayWidth() / 2, DisplayHeight() / 2, 0);
...
static void Task_Scene1_PanUp(u8 taskId)
{
    if (gIntroFrameCounter < TIMER_END_PAN_UP)
    {
        ...
        SetGpuBackgroundY(2, gTasks[taskId].tBg2PosHi);
        ...
        SetGpuBackgroundY(1, gTasks[taskId].tBg1PosHi);
        ...
        SetGpuBackgroundY(0, gTasks[taskId].tBg3PosHi);

        if (gIntroFrameCounter == TIMER_FLYGON_SILHOUETTE_APPEAR)
        {
            // Show Flygon silhouette
            u8 spriteId = CreateSprite(&sSpriteTemplate_FlygonSilhouette, DisplayWidth() / 2, DisplayHeight(), 10);
...
static void Task_Scene2_CreateSprites(u8 taskId)
{
    ...
    CreateSprite(&sSpriteTemplate_Manectric, DisplayWidth() + 32, 128, 0);
    CreateSprite(&sSpriteTemplate_Torchic, DisplayWidth() + 48, 110, 1);

    if (sIntroCharacterGender == MALE)
        spriteId = CreateIntroBrendanSprite(DisplayWidth() + 32, 100);
    else
        spriteId = CreateIntroMaySprite(DisplayWidth() + 32, 100);

    gSprites[spriteId].callback = SpriteCB_PlayerOnBicycle;
    gSprites[spriteId].anims = sAnims_PlayerBicycle;
    gTasks[taskId].tPlayerSpriteId = spriteId;
    CreateSprite(&sSpriteTemplate_Volbeat, DisplayWidth() + 32, 80, 4);
...
static void Task_Scene3_Load(u8 taskId)
{
    ...
    PanFadeAndZoomScreen(DisplayWidth() / 2, DisplayHeight() / 2, 0, 0);
    ResetSpriteData();
    FreeAllSpritePalettes();
    BeginNormalPaletteFade(PALETTES_ALL, 0, 16, 0, RGB_WHITEALPHA);
    ClearGpuBackgroundState(2);
...
static void Task_Scene3_SpinPokeball(u8 taskId)
{
    ...
    PanFadeAndZoomScreen(DisplayWidth() / 2, DisplayHeight() / 2, SAFE_DIV(0x10000, gTasks[taskId].tZoomDiv), gTasks[taskId].tAlpha);
...
static void Task_Scene3_InitGroudonBg(u8 taskId)
{
    ...
    SetGpuWindowOut(0);
    ClearGpuBackgroundState(2);
    SetGpuBackgroundScreenBaseBlock(2, 24);
    SetGpuBackground8bppMode(2, 1);
    SetGpuBackgroundAreaOverflowMode(2, 1);
...
static void Task_Scene3_NarrowWindow(u8 taskId)
{
    ...
    {
        SetGpuWindowY(0, WIN_RANGE(NARROW_HEIGHT, DisplayHeight() - NARROW_HEIGHT));
...
static void SpriteCB_KyogreBubbles(struct Sprite *sprite)
{
    ...
    case 1:
        ...
        if ((u16)(sprite->y - 20) > DisplayHeight() - 20)
            DestroySprite(sprite);
        break;
    }
}
...
static void Task_Scene3_LoadClouds1(u8 taskId)
{
    ...
    SetGpuBackgroundWidth(0, 512);
    SetGpuBackgroundHeight(0, 256);

    ClearGpuBackgroundState(1);
    SetGpuBackgroundPriority(1, 0);
    SetGpuBackgroundCharBaseBlock(1, 1);
    SetGpuBackgroundScreenBaseBlock(1, 26);
    SetGpuBackground8bppMode(1, 0);
    ...
    SetGpuBackgroundScreenBaseBlock(2, 28);
    SetGpuBackground8bppMode(2, 0);
...
static void Task_Scene3_Clouds(u8 taskId)
{
    ...
    SetGpuBackgroundX(0, (tCloudPos >> 8));
    SetGpuBackgroundX(1, -(tCloudPos >> 8));
...
static void Task_RayquazaAttack(u8 taskId)
{
    ...
    case 2:
        if (data[3] == 0)
        {
            ...
            if (data[1] == 6)
            {
                spriteId = CreateSprite(&sSpriteTemplate_RayquazaOrb, DisplayWidth() / 2, 88, 15);
...
static void Task_BlendLogoIn(u8 taskId)
{
    ...
    default:
        SetGpuState(GPU_STATE_BLDCNT, BLDCNT_EFFECT_BLEND
                                   ...
                                   | BLDCNT_TGT2_OBJ
                                   | BLDCNT_TGT2_BD);
        SetGpuState(GPU_STATE_BLDALPHA, gTitleScreenAlphaBlend[31]);
        SetGpuState(GPU_STATE_BLDY, 0);
    ...
    case 1:
        if (gTasks[taskId].data[1] != 0)
        {
            ...
            SetGpuState(GPU_STATE_BLDALPHA, gTitleScreenAlphaBlend[tmp]);
        }
        else
        {
            SetGpuState(GPU_STATE_BLDALPHA, gTitleScreenAlphaBlend[0]);
...
static void SpriteCB_PlayerOnBicycle(struct Sprite *sprite)
{
    ...
    case 2:
        // Move backwards
        if (sprite->x <= (DisplayWidth() / 2) || gIntroFrameCounter & 7)
            sprite->x++;
        break;
    case 3:
        // Bike in place
...
static void SpriteCB_Flygon(struct Sprite *sprite)
{
    ...
    case 0:
        break;
    case 1:
        if (sprite->x2 + sprite->x < DisplayWidth() + 64)
            sprite->x2 += 8;
        else
            sprite->sState = 2;
        break;
    case 2:
        if (sprite->x2 + sprite->x > (DisplayWidth() / 2))
            sprite->x2 -= 1;
        else
            sprite->sState = 3;
        break;
...
static u8 CreateGameFreakLogoSprites(s16 x, s16 y, s16 unused)
{
    ...
    spriteId = CreateSprite(&sSpriteTemplate_GameFreakLogo, DisplayWidth() / 2, y - 6, 0);
    gSprites[spriteId].sState = 0;
    gSprites[spriteId].invisible = TRUE;
    gSprites[spriteId].oam.matrixNum = i + 12;
